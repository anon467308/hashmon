...
let earliest_cell lastev mf =
  ...
  let rec go = function
    ...
    | MOr (_, f1, f2, _)
    | MSinceA (_, _, f1, f2, _)
    | MSince (_, _, _, f1, f2, _) -> go f1; go f2
  in
  go mf; !earliest
...
let comb_m lastev f1 f2 =
  ...
  let rec comb_m f1 f2 = match (f1, f2) with
    ...
    | (MSince (c1, c2, dt, f11, f12, sinf1), MSince(_, _, _, f21, f22, sinf2))
      -> MSince         (c1, c2, dt, comb_m f11 f21, comb_m f12 f22, combine_sinfo sinf1 sinf2)
...
let split_state mapping mf size =
  ...
  *)
  ...
  let rec split_f = function
    ...
    | MSince         (c1, c2, dt, f1, f2, sinf)                 ->
      (*print_endline "since";*)
      let a1 = (split_f f1) in let a2 = (split_f f2) in  Array.mapi (fun i e -> MSince(c1, c2, dt, a1.(i), a2.(i), e)) (split_sinfo sinf (p1 f1) (p1 f2))
    | MOnceA         (dt, f1, oainf)                            ->
      (*print_endline "oncea";*)
...
let split_according_to_modulo keys num_partitions t p  = 
  ...
  if List.length pos = 0 then let res = Array.make num_partitions 0 in Array.mapi (fun i e -> i) res else
  (* Else we create a mapping to reorder our partition input values *)
  let map t = List.map (fun e -> Tuple.get_at_pos t e) pos in
...
let rec print_ef = function
  ...
  | ESince         (c1, c2, dt, f1, f2, sinf)                 -> print_endline "Since";print_ef f1;print_ef f2
