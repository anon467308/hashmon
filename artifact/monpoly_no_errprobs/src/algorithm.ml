...
let update_since_all comp1 intv tsq inf comp2 rel1 rel2 =
  inf.sres <- comp2 inf.sres rel1;
    ...
    if not (Mqueue.is_empty auxrels) then
      ...
      if MFOTL.in_right_ext (MFOTL.ts_minus tsq tsj) intv then
        begin
          ...
          inf.sres <- Relation.union inf.sres (comp2 relj rel1);
          elim ()
        end
  in
  elim ();
  ...
  Mqueue.update_and_delete
    (fun (tsj, relj) -> (tsj, comp2 relj rel1))
    (fun (_,relj) -> Relation.is_certainly_empty relj) (* delete the current node if newrel is empty *)
    auxrels;

  if not (Relation.is_certainly_empty rel2) then
    begin
...
let update_since comp1 intv tsq auxrels comp discard rel1 rel2 =
  ...
  Mqueue.update_and_delete
    (fun (tsj,relj) ->
       ...
       if (not discard) && MFOTL.in_right_ext (MFOTL.ts_minus tsq tsj) intv then
         res := Relation.union !res (comp1 newrel);
       (tsj,newrel)
    )
    (* delete the current node if newrel is empty *)
    (fun (_,relj) -> Relation.is_certainly_empty relj)
    auxrels;

  if not (Relation.is_certainly_empty rel2) then
    begin
      if (not discard) && MFOTL.in_right_ext MFOTL.ts_null intv then
        res := Relation.union !res (comp1 rel2);
      mqueue_add_last auxrels tsq rel2
    end;
...
let update_old_until comp1 q tsq i intv inf discard  =
  ...
  Sj.iter (
    fun (j,tsj,rrels) ->
      ...
      if (not discard) && not (Relation.is_certainly_empty relq) then
        inf.ures <- Relation.union inf.ures (comp1 relq);
      if Misc.debugging Dbg_eval then
        Relation.print_reln "[update_aux] res: " inf.ures;
  ) inf.raux;

(*let rec elim_old_err1 () =
    if not (Sk.is_empty inf.uerr1) then
      let (k, _) = Sk.get_first inf.uerr1 in
      if k < q then
        begin
          ignore (Sk.pop_first inf.uerr1);
          elim_old_err1 ()
        end
  in
  elim_old_err1 ();

  let rec elim_old_err2 () =
    if not (Sj.is_empty inf.uerr2) then
      let (j, tsj, _) = Sj.get_first inf.uerr2 in
      if j < q || not (MFOTL.in_right_ext (MFOTL.ts_minus tsj tsq) intv) then
        begin
          ignore (Sj.pop_first inf.uerr2);
          elim_old_err2 ()
        end
  in
  elim_old_err2 ();*)
...
let combine2 comp j rels rel2 =
  ...
  Sk.iter
    (fun (k,rel) ->
       ...
       if not (Relation.is_certainly_empty nrel) then
         Sk.add_last (k,nrel) nrels;
       curr_rel2 := Relation.diff None !curr_rel2 nrel;
    ) rels;
  if not (Relation.is_certainly_empty !curr_rel2) then
...
let update_until comp1 q tsq i tsi intv rel1 rel2 inf comp discard =
  if Misc.debugging Dbg_eval then
    print_uinf "[update_until] inf: " inf;
  assert(i >= q);
  let nsaux = combine2 (Relation.inter None) i inf.saux rel1 in
  if (MFOTL.in_right_ext (MFOTL.ts_minus tsi tsq) intv) &&
     not (Relation.is_certainly_empty rel2) then
    begin
      let rrels = combine2 comp i inf.saux rel2 in
      Sj.add_last (i,tsi,rrels) inf.raux;
      (*Sj.add_last (i, tsi, Relation.err_info_of rel2) inf.uerr2;*)
      if not discard then
        match get_relq q rrels with
        | Some rel -> inf.ures <- Relation.union inf.ures (comp1 rel)
        | None -> ()
    end;
  inf.saux <- nsaux(*;
  if not (Relation.is_certainly_empty rel1) then
    Sk.add_last (i, Relation.err_info_of rel1) inf.uerr1*)

(*let compute_until_error q tsq intv inf =
  let start_iter l = if Dllist.is_empty l then None
    else Some (Dllist.get_first_cell l) in
  let peek = function
    | None -> None
    | Some c -> Some (Dllist.get_data c)
  in
  let next l = function
    | None -> None
    | Some c -> if Dllist.is_last l c then None else Some (Dllist.get_next l c)
  in
  let cur1 = ref (start_iter inf.uerr1) in
  let err1 = ref None in
  let rec catch_up j =
    match peek !cur1 with
      | None -> ()
      | Some (k, err) ->
          if k < j then
            begin
              (match !err1 with
                | None -> err1 := Some err
                | Some x -> err1 := Some (Relation.inter_err_info x err));
              cur1 := next inf.uerr1 !cur1;
              catch_up j
            end
  in
  let rec loop cur2 result =
    match peek cur2 with
      | None -> result
      | Some (j, tsj, err) ->
          begin
            catch_up j;
            let result =
              if MFOTL.in_interval (MFOTL.ts_minus tsj tsq) intv then
                let err' = match !err1 with
                  | None -> err
                  | Some x -> Relation.natural_join_sc2_err_info None err x
                in
                Relation.union_err_info result err'
              else result
            in
            loop (next inf.uerr2 cur2) result
          end
  in
  loop (start_iter inf.uerr2) Relation.empty_err_info*)
...
let add_let_index f n rels =
  let rec update = function
    ...
    | EAnd (_,f1,f2,_)
    | EOr (_,f1,f2,_)
    | ESinceA (_,_,_,f1,f2,_)
    | ESince (_,_,_,f1,f2,_)
    | ENUntil (_,_,_,_,f1,f2,_)
...
let rec eval f crt discard =
  ...
  | ENeg f1 ->
    (match eval f1 crt discard with
     | Some rel -> Some (Relation.negate rel)
     | None -> None
    )
  ...
  | EAnd (comp,f1,f2,inf) ->
    ...
    let eval_and rel1 =
      if Relation.is_certainly_empty rel1 then
  ...
  | ESinceA (comp1,comp2,intv,f1,f2,inf) ->
    if Misc.debugging Dbg_eval then
    ...
    let update_sauxrels = update_since_all comp1 intv tsq inf comp2 in
  ...
  | ESince (comp1,comp2,intv,f1,f2,inf) ->
    if Misc.debugging Dbg_eval then
    ...
    let update_sauxrels = update_since comp1 intv tsq inf.sauxrels comp2 discard in
  ...
  | EUntil (comp1,comp2,intv,f1,f2,inf) ->
    ...
    *)
    ...
    if inf.ufirst then
      begin
        ...
        update_old_until comp1 q tsq i intv inf discard;
        if Misc.debugging Dbg_eval then
    ...
    let rec evalf1 i tsi rel2 ncrt =
      (match eval f1 ncrt false with
       | Some rel1 ->
         update_until comp1 q tsq i tsi intv rel1 rel2 inf comp2 discard;
         inf.urel2 <- None;
         inf.ulast <- ncrt;
         evalf2 ()
       | None ->
      ...
      if Neval.is_last inf.ulast then
        ...
        if not (MFOTL.in_left_ext (MFOTL.ts_minus tsi tsq) intv) then
          ...
          begin
            ...
            (*let err = compute_until_error q tsq intv inf in
            let res = Relation.with_err_info err inf.ures in*)
            let res = inf.ures in
  ...
  | ENUntil (comp1,comp2,ispos,intv,f1,f2,inf) ->
      ...
      begin
        ...
        let update_f1 relj =
          f1union :=
            ...
            | Some rel ->
              if ispos then
                Some (Relation.inter None rel relj)
              else
                Some (Relation.union rel relj))
        in
              ...
              if MFOTL.in_left_ext (MFOTL.ts_minus tsj2 tsq) intv then
                begin
                  let resj =
                    (match !f1union with
                    | None -> rel2
                    | Some rel -> comp2 rel2 rel)
                  in
                  res := Relation.union !res (comp1 resj);
                  update_f1 rel1;
...
let add_ext neval f =
  ...
  | Let (p, f1, f2) ->
    ...
    let comp = Relation.reorder Relation.no_hash None new_pos in
  ...
  | Exists (vl, f1) ->
    ...
    let pos = List.filter_map (fun v ->
      ...
      with Not_found -> None) vl in
    let pos = List.sort Stdlib.compare pos in
    let comp = Relation.project_away Relation.no_hash None pos in
    EExists (comp,ff1)
  ...
  | Or (f1, f2) ->
    ...
    let comp =
      if attr1 = attr2 then
        ...
        (* first reorder rel2 *)
        (fun rel1 rel2 ->
           let rel2' = Relation.reorder Relation.no_hash None new_pos rel2 in
    ...
    let comp =
      if f2_is_special then
        if Misc.subset attr2 attr1 then
          ...
          fun rel1 _ -> Relation.filter Relation.Exact None filter_cond rel1
        else
          let process_tuple = Tuple.get_tf attr1 f2 in
          fun rel1 _ -> Relation.map {fn = process_tuple; hashed = Relation.no_hash} rel1
      else
        match f2 with
        | Neg _ ->
          if attr1 = attr2 then
            fun rel1 rel2 -> Relation.diff None rel1 rel2
          else
            begin
              assert(Misc.subset attr2 attr1);
              let posl = List.map (fun v -> Misc.get_pos v attr1) attr2 in
              fun rel1 rel2 -> Relation.minus None posl rel1 rel2
            end
        ...
        | _ ->
          ...
          if attr1 = attr2 then
            fun rel1 rel2 -> Relation.inter None rel1 rel2
          else if Misc.subset attr1 attr2 then
            fun rel1 rel2 -> Relation.natural_join_sc1 Relation.no_hash None matches2 rel1 rel2
          else if Misc.subset attr2 attr1 then
            fun rel1 rel2 -> Relation.natural_join_sc2 None matches1 rel1 rel2
          else
            fun rel1 rel2 -> Relation.natural_join Relation.no_hash None matches1 matches2 rel1 rel2
    in
  ...
  | Aggreg (t_y, y, op, x, glist, Once (intv, f)) ->
    ...
    let state =
      match op with
      | Cnt -> Aggreg.cnt_once Relation.no_hash_map default intv 0 posG
      | Min -> Aggreg.min_once Relation.no_hash_map default intv 0 posx posG
      | Max -> Aggreg.max_once Relation.no_hash_map default intv 0 posx posG
      | Sum -> Aggreg.sum_once Relation.no_hash_map default intv 0 posx posG
      | Avg -> Aggreg.avg_once Relation.no_hash_map default intv 0 posx posG
      | Med -> Aggreg.med_once Relation.no_hash_map default intv 0 posx posG
    in
  ...
  | Aggreg (t_y, y, op, x, glist, f)  ->
    ...
    let comp =
      match op with
      | Cnt -> Aggreg.cnt Relation.no_hash_map default 0 posG
      | Sum -> Aggreg.sum Relation.no_hash_map default 0 posx posG
      | Min -> Aggreg.min Relation.no_hash_map default 0 posx posG
      | Max -> Aggreg.max Relation.no_hash_map default 0 posx posG
      | Avg -> Aggreg.avg Relation.no_hash_map default 0 posx posG
      | Med -> Aggreg.med Relation.no_hash_map default 0 posx posG
    in
  ...
  | Since (intv,f1,f2) ->
    ...
    if snd intv = Inf then
      ...
      ESinceA ((fun rel -> rel), comp,intv,ff1,ff2,inf)
    else
      let inf = {srel2 = None; sauxrels = Mqueue.create()} in
      ESince ((fun rel -> rel), comp,intv,ff1,ff2,inf)
  ...
  | Until (intv,f1,f2) ->
    ...
    if neg then
      ...
      in
      ENUntil ((fun rel -> rel), comp,false,intv,ff1,ff2,inf)
    else
      let comp =
        let matches2 = Table.get_matches attr2 attr1 in
        fun relj rel1 -> Relation.natural_join_sc2 None matches2 relj rel1
      ...
      let inf = {ulast = neval0;
                 ...
                 uerr2 = Sj.empty()}
      in
      EUntil ((fun rel -> rel), comp, intv,ff1,ff2,inf)
...
let split_save filename ff posl i neval last =
  failwith "not supported in this version"
(*
    ...
    | MonpolyData {tp; ts; db} ->
      if ts >= !lastts then
        begin
          ...
          lastts := ts;
          if !Misc.force_gc then
            begin
              Gc.full_major ();
              Misc.update_gc_stats ()
            end;
          if Misc.debugging Dbg_extf_stats then
            Extformula.print_stats "[Algorithm.check_log] " ff;
          if cont then
            loop ffl (i + 1)
          else
            finish ()
...
let monitor_extformula logfile fv (extf, extf_fv, neval, lastev) =
  (* compute permutation for output tuples *)
  let fv_pos = List.map snd (Table.get_matches extf_fv fv) in
  assert (List.length fv_pos = List.length fv);
  let lexbuf = Log.log_open logfile in
  check_log lexbuf extf fv_pos neval 0 lastev
