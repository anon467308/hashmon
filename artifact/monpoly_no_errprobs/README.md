...
tested with version 4.12.0 of [OCaml](https://ocaml.org) under Linux. It should
...
Usage:
```
monpoly [options...] -sig <file> -formula <file> [-negate] [-log <file>]
```
...
```
  ...
  -no_rw        Disable formula rewriting
  -verified     Run Monpoly's verified kernel
  -no_mw        No multi-way join (only with the verified kernel)
  -unfold_let {no|full|smart}
                Control unfolding of LET expressions in the formula (default 'no')
  -hash {bypass|off|single[,bits]|merged[,bits]}
                Hashing mode, explained below (default 'bypass')
  -analyze      Print rewritten expression, analyze error probabilities from
                hashing and exit
  -errprobs     Print estimates of error probabilities
  -seed         Set random generator seed
```
Use `-help` to see all supported options.

To enable randomized monitoring with hashing, pass either the `-hash single` or
the `-hash merged` option. The latter merges the hashes of multiple variables
where it is possible, which can further reduce the memory usage. Use `-analyze`
to print the temporal-relational algebra expression being monitored, which shows
the exact place where hashing takes place. It also shows the result of the
static error probability analysis as symbolic expressions. The following
variables are used in the expressions:
  * FP: false-positive probability, per time-point
  * FN: false-negative probability, per time-point
  * rate: maximum number of time-points per time unit
  * input.R: maximum number of tuples in input relation R, per time-point
  * size.X.Y.Z: maximum number of tuples satisfying the subformula at path
    X.Y.Z, per time-point. For example, if the formula has the shape
    "alpha AND EX x. beta", then the path "and.2.ex" refers to subformula beta.

Randomized monitoring may result in incorrect verdicts. Use `-errprobs` to
annotate the verdicts with upper bounds on the false-positive and false-negative
probabilities, based on the current input log file. By default, 63-bits hashes
are used, which is the maximum size. The impact of shorter hashes can be
simulated by specifying, e.g., `-hash merged,N`, where N is the hash size in
bits. This does *not* affect memory usage.

The `off` mode differs from the default `bypass` mode in that the same rewriting
steps and code paths as in `-hash single` are taken, except that the hash
function is replaced by the identity mapping.

Note that `-verified` is incompatible with hashing.
...
monpoly-dev.opam        OPAM package description (generated by Dune)
...
/src                    Directory with the source code
  ...
  hashing.ml[i]         Module for inserting hashing operations
  sliding.ml[i]         Module implementing the sliding window algorithm
  aggreg.ml[i]          Module implementing aggregation operators
  hashing.ml[i]         Module for inserting and analyzing hashing operators
...
/contrib                Directory with third-party libraries
  /clhash               Directory with CLHASH source code, modified to build static library
```
