...
let show_error_estimate = ref false

let force_gc = ref false
let max_gc_words = ref 0
let max_gc_blocks = ref 0
...
let update_gc_stats () =
  ...
  max_gc_words := max !max_gc_words stats.live_words;
  max_gc_blocks := max !max_gc_blocks stats.live_blocks
