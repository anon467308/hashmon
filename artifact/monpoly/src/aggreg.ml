...
let empty_result empty_val group_posl =
  if group_posl = [] then
    (true, Tuple_set.singleton (Tuple.make_tuple [empty_val]))
  else
    (false, Tuple_set.empty)

let nonempty_result rel = (false, rel)

type aggregator = Relation.relation -> result

let comp_aggreg ~mult empty_val init update post tuple_map result_pos group_posl rel =
  let empty_rel, set =
      ...
      begin
        ...
        Hashtbl.iter
          (fun gtuple v ->
            ...
            new_rel := Tuple_set.add Relation.(tuple_map.fn rtuple) !new_rel
          ) map;
        nonempty_result !new_rel
      end
  in
  {empty_rel;
   rel = Relation.approx_agg ~mult tuple_map.hashed group_posl rel set}

let cnt tuple_map empty_val result_pos group_posl =
  let init _t = 1 in
  let update ov _nt = ov + 1 in
  let post c = Int c in
  comp_aggreg ~mult:true empty_val init update post tuple_map result_pos group_posl

let min tuple_map empty_val result_pos arg_pos group_posl =
  let init t = Tuple.get_at_pos t arg_pos in
  let update ov nt = min ov (Tuple.get_at_pos nt arg_pos) in
  comp_aggreg ~mult:false empty_val init update (fun x -> x) tuple_map result_pos group_posl

let max tuple_map empty_val result_pos arg_pos group_posl =
  let init t = Tuple.get_at_pos t arg_pos in
  let update ov nt = max ov (Tuple.get_at_pos nt arg_pos) in
  comp_aggreg ~mult:false empty_val init update (fun x -> x) tuple_map result_pos group_posl

let sum tuple_map empty_val result_pos arg_pos group_posl =
  let init t = Tuple.get_at_pos t arg_pos in
  let update ov nt = plus ov (Tuple.get_at_pos nt arg_pos) in
  comp_aggreg ~mult:true empty_val init update (fun x -> x) tuple_map result_pos group_posl

let avg tuple_map empty_val result_pos arg_pos group_posl =
...
let med tuple_map empty_val result_pos arg_pos group_posl =
  let init t = ([Tuple.get_at_pos t arg_pos], 1) in
...
class comm_group_aggregator init add remove post tuple_map intv empty_val
        ...
        if not (Queue.is_empty tw_rels) then
          ...
          if not (MFOTL.in_left_ext diff intv) then
            begin
              ...
              Tuple_set.iter evict (Relation.to_set rel);
              tracker := Relation.del_approx_error !tracker rel;
              loop ()
            end
      in
      loop ()
        ...
        if Hashtbl.length acc = 0 then empty_result empty_val group_posl
          ...
          begin
            ...
            Hashtbl.iter
              (fun gtuple v ->
                let rtuple = Tuple.insert result_pos gtuple (post v) in
                res := Tuple_set.add Relation.(tuple_map.fn rtuple) !res
              ) acc;
            nonempty_result !res
          end
      in
      {empty_rel;
       rel = Relation.approx_agg ~mult:true Relation.(tuple_map.hashed)
        group_posl !tracker set}
  end
...
let cnt_once tuple_map empty_val intv result_pos group_posl =
  let init _t = 1 in
  let add ov _nt = ov + 1 in
  let remove ov _dt = if ov = 1 then None else Some (ov - 1) in
  let post x = Int x in
  let window = new comm_group_aggregator init add remove post tuple_map intv
    empty_val result_pos group_posl in
  new once_aggregator window intv

let sum_avg_once post tuple_map empty_val intv result_pos arg_pos group_posl =
  let init t = (Tuple.get_at_pos t arg_pos, 1) in
...
let med_once tuple_map empty_val intv result_pos arg_pos group_posl =
...
class mono_aggregator is_better tuple_map intv empty_val result_pos arg_pos group_posl =
  ...
  object
    ...
    method add_rel ts rel =
      if not (MFOTL.infinite_interval intv) then
        begin
          Queue.push (ts, Relation.strip_set rel) tw_rels;
          tracker := Relation.add_approx_error !tracker rel
        end;
      (* The invariant is:
    ...
    method get_result =
      ...
      in
      {empty_rel;
       rel = Relation.approx_agg ~mult:false Relation.(tuple_map.hashed)
         group_posl !tracker set}
  end

let min_once tuple_map empty_val intv result_pos arg_pos group_posl =
  let is_better x y = -(Stdlib.compare x y) in
  let window = new mono_aggregator is_better tuple_map intv empty_val
    result_pos arg_pos group_posl in
  new once_aggregator window intv

let max_once tuple_map empty_val intv result_pos arg_pos group_posl =
  let is_better x y = Stdlib.compare x y in
  let window = new mono_aggregator is_better tuple_map intv empty_val
    result_pos arg_pos group_posl in
  new once_aggregator window intv
