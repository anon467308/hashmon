...
module ListImpl = struct
  ...
  let string_of_tuple = Misc.string_of_list (string_of_cst true)
  let print_tuple = Misc.print_list (print_cst true)
end


module ArrayImpl = struct
  type tuple = cst array
  ...
  let make_tuple x = Array.of_list x
  ...
  let get_constants tuple = Array.to_list tuple

  let get_at_pos = Array.get

  let insert pos tuple v =
    let n = Array.length tuple in
    Array.init (n+1) (fun i ->
      if i < pos then Array.get tuple i
      else if i = pos then v
      else Array.get tuple (i-1))

  let add_first tuple v = insert 0 tuple v
  let add_last tuple v = insert (Array.length tuple) tuple v

  let project_away pos t = make_tuple (Misc.remove_positions pos (Array.to_list t))
  let projections pos t = make_tuple (Misc.get_positions pos (Array.to_list t))
  ...
  (* constants. TODO: do we really? Anyhow, terms should be simplified. *)
  let satisfiesp arg_list tuple =
    let rec satisf assign res crt args constr =
      match args with
      | [] ->
        if check_constr assign constr then
          true, Array.of_list (List.rev res)
        else
          false, [||]

      | term :: args' ->
        let c = Array.get tuple crt in
        (match term with
         | Var x ->
           (try
              let c' = List.assoc x assign in
              if c = c' then
                satisf assign res (crt+1) args' constr
              else
                false, [||]
            with Not_found ->
              satisf ((x, c) :: assign) (c :: res) (crt+1) args' constr
           )
         ...
         | _ -> satisf assign res (crt+1) args' ((c, term) :: constr)
        )
    in
    satisf [] [] 0 arg_list []
  ...
  let join posval t1 t2 =
    let t1 = Array.to_list t1 in
    let t2 = Array.to_list t2 in
    let rec join' crtpos pv t =
      match pv,t with
      | (hp,hv)::tpv, ht::tt ->
        if hp = crtpos then
      ...
      | _,[] -> failwith "[Tuple.join] bad posval list"
    in
    make_tuple (t1 @ (join' 0 posval t2))
  ...
  let join_rev pos2 posval t2 t1 =
    ...
    in
    check 0 posval t1;
    make_tuple (t1 @ (sel 0 pos2 t2))


  (** printing functions **)

  let string_of_tuple t = Misc.string_of_list (string_of_cst true)
    (Array.to_list t)

  let print_tuple t = Misc.print_list (print_cst true) (Array.to_list t)
end
