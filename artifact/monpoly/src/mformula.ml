...
type muinfo  = { mulast   :  Neval.cell;
                 ...
                 muerr1   : (int * err_info) Sk.dllist;
                 muerr2   : (int * timestamp * err_info) Sj.dllist}
...
type mformula =
  ...
  | MSinceA of comp_one * comp_two * interval * mformula * mformula * sainfo
  | MSince of comp_one * comp_two * interval * mformula * mformula * sinfo
  | MOnceA of interval * mformula * oainfo
  | MOnceZ of interval * mformula * mozinfo
  | MOnce of interval * mformula  * moinfo
  | MNUntil of comp_one * comp_two * bool * interval * mformula * mformula * muninfo
  | MUntil of comp_one * comp_two * interval * mformula * mformula * muinfo
...
let free_vars f =
  ...
  | MSinceA        (_, _, _, f1, f2, _)   -> Misc.union (get_pred f1) (get_pred f2)
  | MSince         (_, _, _, f1, f2, _)   -> Misc.union (get_pred f1) (get_pred f2)
  | MOnceA         (_, f1, _)             -> get_pred f1
  | MOnceZ         (_, f1, _)             -> get_pred f1
  | MOnce          (_, f1, _)             -> get_pred f1
  | MNUntil        (_, _, _, _, f1, f2, _)-> Misc.union (get_pred f1) (get_pred f2)
  | MUntil         (_, _, _, f1, f2, _)   -> Misc.union (get_pred f1) (get_pred f2)
